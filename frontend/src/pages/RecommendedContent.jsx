...
const RecommendedContent = () => {
    ...
    return (
        <>
            ...
            <LoggedinNav />
            <div className={`bg-[#0a0a0a] p-6 rounded-lg shadow-xl max-w-2xl mx-auto mt-4 ${isSidebarOpen ? "md:ml-[30%]" : "md:ml-[30%]"}`}>

<h2 className="text-xl font-semibold text-white mb-4 text-center">Recommended Content</h2>
<p className="text-white mb-4 text-center">Use our powerful recommendation system to find the best team or teammates!</p>

{/* Selection buttons */}
<div className="text-center mb-4">
    <button
        onClick={() => setSelectedOption("teams")}
        className={`px-4 py-2 ${selectedOption === "teams" ? "bg-blue-500" : "bg-[#242424]"} text-white rounded-md transition-all duration-300`}
    >
        Teams
    </button>
    <button
        onClick={() => setSelectedOption("users")}
        className={`ml-4 px-4 py-2 ${selectedOption === "users" ? "bg-blue-500" : "bg-[#242424]"} text-white rounded-md transition-all duration-300`}
    >
        Users
    </button>
</div>
...
{selectedOption === "users" && (
    <div>
        ...
        {selectedTeam && recommendedUsers.length > 0 ? (
            <ul className="space-y-4 mt-4 max-h-[70vh] overflow-y-auto">
                {recommendedUsers.map((user, index) => (
                    ...
                    >
                        ...
                        <p className="text-gray-300 text-sm mt-1">Role: {user.role}</p>
            ...
            <p className="text-gray-300 text-center mt-4">No recommended users found.</p>
        ) : null}
    </div>
)}
</div>
        ...
        <div className="bg-[#0a0a0a] p-6 rounded-lg shadow-lg w-96">
            ...
            {myTeams.length > 0 ? (
                <ul className="space-y-2">
                    {myTeams.map((team) => (
                        <li key={team.id} className="flex justify-between items-center p-3 bg-[#141414] rounded-lg">
