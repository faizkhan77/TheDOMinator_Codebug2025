...
const RecommendedContent = () => {
  ...
  useEffect(() => {
    if (selectedOption === "teams") {
      ...
      // Updated: Fetch user's teams from the authenticated user's profile
      fetchUserTeamsFromProfile();
    }
  }, [selectedOption]);

  // Added to ensure the component has the user's teams upon load
  useEffect(() => {
    fetchUserTeamsFromProfile();
  }, []);
  ...
  const fetchRecommendedTeams = async () => {
    ...
    try {
      ...
      setError(
        "Failed to load recommendations. Make sure you are logged in and have a complete profile."
      );
    } finally {
      setLoading(false);
    }
  ...
  // --- REVISED: Fetch User Teams from Local Storage ---
  const fetchUserTeamsFromProfile = () => {
    setLoading(true);
    setError(null);

    const userProfile = JSON.parse(localStorage.getItem("userProfile"));
    if (userProfile && userProfile.teams) {
      // Assuming userProfile.teams holds the array of teams as fetched by the backend serializer
      // If the serializer is TeamSerializer, it returns the whole team object.
      // If it is just PrimaryKeyRelatedField, it returns IDs.
      // We need the full team object to display the name in the dropdown.

      // Fetch the full list of teams from the backend to ensure we have the full team data
      const token = localStorage.getItem("access");
      axios
        .get("/api/teams/", { headers: { Authorization: `Bearer ${token}` } })
        .then((response) => {
          const allTeams = response.data;
          // Filter the full list to only include teams the user is a part of
          const myTeamIds = userProfile.teams.map((teamId) => teamId);
          const myTeamsFullData = allTeams.filter((team) =>
            myTeamIds.includes(team.id)
          );
          setUserTeams(myTeamsFullData);
        })
        .catch((err) => {
          console.error("Failed to fetch user's full teams:", err);
          setError("Failed to load your teams. Please refresh.");
        })
        .finally(() => {
          setLoading(false);
        });
    } else {
      setError("User profile or teams list not found.");
      setLoading(false);
    }
  };
  ...
  const sendInvitation = async (teamId) => {
    ...
    try {
      // Note: Your backend serializer expects recipient_id and team_id for Invitation create
      await axios.post(
        "/api/invitations/",
        {
  ...
  return (
    <>
      ...
      <LoggedinNav />
      <div
        className={`bg-[#0a0a0a] p-6 rounded-lg shadow-xl max-w-2xl mx-auto mt-4 ${
          isSidebarOpen ? "md:ml-[30%]" : "md:ml-[30%]"
        }`}
      >
        ...
        <p className="text-white mb-4 text-center">
          Use our powerful recommendation system to find the best team or
          teammates!
        </p>
        ...
        <div className="text-center mb-4">
          <button
            ...
            className={`px-4 py-2 ${
              selectedOption === "teams" ? "bg-blue-500" : "bg-[#242424]"
          ...
          <button
            ...
            className={`ml-4 px-4 py-2 ${
              selectedOption === "users" ? "bg-blue-500" : "bg-[#242424]"
            } text-white rounded-md transition-all duration-300`}
          >
            Users
          </button>
          ...
          <ul className="space-y-4 max-h-[70vh] overflow-y-auto">
            {recommendedTeams.map((team, index) => (
              <li
                ...
                className="bg-[#141414] p-4 rounded-xl shadow-lg hover:shadow-2xl hover:bg-[#1e1e1e] transition duration-300"
              >
                <Link
                  to={`/team/${team.id}`}
                  state={{ team }}
          ...
          <div>
            ...
            {userTeams.length > 0 ? (
              <select
                ...
                className="bg-[#141414] text-white p-2 rounded-md w-full"
              >
              ...
              <p className="text-gray-300 text-center">No teams found.</p>
            )}
                ...
                {recommendedUsers.map((user, index) => (
                  ...
                  >
                    ...
                    <p className="text-gray-400 text-sm mt-1">
                      Skills:{" "}
                      {user.skills.map((skill) => skill.skill_name).join(", ")}
                    </p>
      ...
      {isModalOpen && selectedUser && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
          <div className="bg-[#0a0a0a] p-6 rounded-lg shadow-lg w-96">
            ...
            {myTeams.length > 0 ? (
              <ul className="space-y-2">
                {myTeams.map((team) => (
                  <li
                    key={team.id}
                    className="flex justify-between items-center p-3 bg-[#141414] rounded-lg"
                  >
                    <span className="text-white">{team.name}</span>
                    <button
                      onClick={() => sendInvitation(team.id)}
                      className="px-3 py-1 bg-green-500 hover:bg-green-600 text-white rounded transition duration-300"
                    >
                      Invite
                    </button>
                  </li>
            ...
            <button
              onClick={closeModal}
              className="mt-4 px-4 py-2 bg-red-500 hover:bg-red-600 text-white rounded-lg w-full transition duration-300"
            >
              Cancel
            </button>
          </div>
